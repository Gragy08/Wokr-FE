...
// Đăng ký plugins để kiểm tra loại tệp
registerPlugin(
...
export const FormProfile = () => {
    ...
    // Filepond luôn luôn trả ra 1 mảng hình ảnh dù chỉ cho phép upload tối đa 1 ảnh đi nữa
    ...
    useEffect(() => {
        if(infoUser) {
            if(infoUser.avatar) {
                setAvatars([
                    {
                        source: infoUser.avatar, // Đường dẫn đến hình ảnh
                    }
                ])
            }
    ...
    const handleSubmit = (event: any) => {
        ...
        let avatar = null;
        if(avatars.length > 0) {
            avatar = avatars[0].file; // Lấy tệp đầu tiên trong mảng avatars
        }
            ...
            {infoUser && (
                <form onSubmit={handleSubmit} id="profileForm" action="" className="grid sm:grid-cols-2 grid-cols-1 gap-x-[20px] gap-y-[15px]">
                    ...
                    <div className="sm:col-span-2">
                        ...
                        <FilePond
                            name="avatar"
                            allowMultiple={false} // Chỉ cho phép tải lên một tệp
                            allowRemove={true} // Cho phép xóa tệp đã tải lên
                            labelIdle='+'
                            acceptedFileTypes={["image/*"]} // Chỉ cho phép tải lên hình ảnh
                            files={avatars}
